...
class Order(models.Model):
    ...
    total_amount = models.DecimalField(_("total amount order"), max_digits=10, decimal_places=2, blank=True, null=True)
...
class Rating(models.Model):
    ...
    global_value = models.IntegerField(_("global rating"), choices=[(i, i) for i in range(1, 11)])
    quality = models.IntegerField(_("quality rating"), choices=[(i, i) for i in range(1, 11)])
    delivery = models.IntegerField(_("delivery rating"), choices=[(i, i) for i in range(1, 11)])
    foto_quality = models.IntegerField(_("foto quality rating"), choices=[(i, i) for i in range(1, 11)])
    description_quality = models.IntegerField(_("description quality"), choices=[(i, i) for i in range(1, 11)])
    value = models.IntegerField(_("another rating"), choices=[(i, i) for i in range(1, 11)])
    created_at = models.DateTimeField(_("create"), auto_now_add=True)
    updated_at = models.DateTimeField(_("update"), auto_now=True)
